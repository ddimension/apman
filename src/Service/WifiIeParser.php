...
class WifiIeParser
{
     ...
     * @return array
     */
    public function parseSignature(string $sig, $type = 'assoc')
    {
        $tags = [];
        if ('wifi4' != substr($sig, 0, 5)) {
            return $tags;
        }
        $td = explode('|', $sig);
        $ies = null;
        foreach ($td as $data) {
            if (substr($data, 0, strlen($type) + 1) != $type.':') {
                continue;
            }
    ...
    public function getResolveIeNames(array $tags)
    {
        ...
        foreach ($tags as $tagId => $tagValue) {
            if (!array_key_exists($tagId, $ie_map)) {
                continue;
            }
            if (221 == $tagId) {
                if (!array_key_exists($tagId, $ieList)) {
                    $ieList[$tagId] = [];
                }
                foreach ($tagValue as $vendorData) {
                    ...
                    if (array_key_exists($vendor, $macdb)) {
                        $vendorSet['Vendor-Name'] = $macdb[$vendor];
                    }
                    switch ($vendor) {
                        case '0050f2':
                            // Microsoft
