...
class SyslogSocketServer
{
    ...
    public function __construct($host = 'localhost', $port = 9000, \Psr\Log\LoggerInterface $logger, \Doctrine\Persistence\ManagerRegistry $doctrine, \ApManBundle\Service\AccessPointService $apsrv)
    {
    ...
    public function checkDisconnect()
    {
        foreach ($this->changed as $changed_socket) {
            ...
            if (false !== $buf) { // check disconnected client
                continue;
            }
    ...
    public function checkMessageReceived()
    {
    ...
    public function saveMessage($source, $msg)
    {
        if (strlen($msg) < 1) {
            return;
        }
